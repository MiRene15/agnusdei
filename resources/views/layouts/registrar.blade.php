...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family:'Poppins', sans-serif; background:#f4f7fb; margin:0; min-height:100vh; color:#1e293b; padding-left:260px; }
        .sidebar { width:260px; min-width:260px; position:fixed; top:0; left:0; height:100vh; overflow-y:auto; background:linear-gradient(180deg, #001e82 0%, #0c2fa0 100%); padding:28px 18px; display:flex; flex-direction:column; justify-content:flex-start; box-shadow:6px 0 20px rgba(0,0,0,0.08); z-index:20; }
        ...
        .sidebar a, .sidebar button { color:#ffffff; text-decoration:none; padding:14px 16px; border-radius:14px; margin-bottom:12px; font-size:14px; transition:0.25s ease; border:none; background:transparent; text-align:left; width:100%; font-family:inherit; cursor:pointer; display:block; line-height:1.5; box-sizing:border-box; }
        .sidebar a:hover, .sidebar button:hover { background:rgba(255,255,255,0.16); }
        .sidebar a.active { background:#ffffff; color:#001e82; font-weight:700; box-shadow:0 10px 20px rgba(0,0,0,0.10); }
        .logout-wrap { margin-top:auto; padding-top:12px; }
        .logout-btn { background:rgba(255, 8, 49, 0.908) !important; }
        ...
        .btn-danger:hover { transform:translateY(-1px); box-shadow:0 16px 28px rgba(153, 27, 27, 0.18); }
        ...
        @media (max-width: 991px) { body { padding-left:220px; } .grid-2, .grid-3 { grid-template-columns:1fr; } .sidebar { width:220px; min-width:220px; } .topbar { padding:18px 20px; } .content { padding:20px; } }
        @media (max-width: 768px) { body { padding-left:0; } .sidebar { width:100%; min-width:100%; height:auto; position:relative; } }
    </style>
</head>
<body>
...
<div class="main">
    <div class="topbar"><h3>@yield('title', 'Registrar Dashboard')</h3><div class="welcome">Welcome, {{ Auth::user()->name ?? 'Registrar' }}</div></div>
